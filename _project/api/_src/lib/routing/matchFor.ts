...
export function matchFor<Rsc extends Record<string, any>>(
  ...
  const matchWithServices2: <Key extends keyof Rsc>(
    action: Key
  ) => MatchWithServices2<Key> = (action) => (services, f2, f) => matchWithServices2_(action, services, f2, f)
